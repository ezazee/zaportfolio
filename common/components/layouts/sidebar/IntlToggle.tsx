...
const IntlToggle = () => {
  ...
  return (
    <div className="flex items-center justify-center">
      <div
        className={`relative hidden items-center gap-1 rounded-full border-[1.5px] border-neutral-300 bg-neutral-100 p-1 dark:border-slate-700 dark:bg-slate-800/80 lg:flex ${
          isPending ? "pointer-events-none opacity-70" : ""
        }`}
        style={{ width: `${totalWidth + (locales.length - 1) * 4 + 10}px` }}
      >
        <motion.div
          className="absolute bottom-1 top-1 w-10 rounded-full bg-[#fbe400]"
          animate={{
        ...
        {locales.map((locale, index) => (
          ...
          >
            <motion.div
              ...
              animate={{
                color: currentIndex === index ? "#020617" : "#94a3b8",
              }}
      ...
      >
        <motion.div
          transition={{ duration: 0.3, ease: "easeInOut" }}
          className="flex h-8 w-8 items-center justify-center rounded-full bg-accent text-slate-900"
        >
