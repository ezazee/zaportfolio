...
const Item = ({ label, value }: ItemProps) => (
  <div className="flex flex-col items-center">
    ...
    <span className="text-2xl text-primary dark:text-neutral-100">{value}</span>
  </div>
);
  ...
  const XpProgress = () => (
    <div className="flex w-full items-center justify-between gap-3">
      <Tooltip title={`${data?.xp} ${t("total_xp")}`}>
        <span className="text-sm font-medium text-primary dark:text-neutral-100">{level}</span>
      </Tooltip>
      ...
      <div className="flex flex-col gap-y-2">
        <div className="flex gap-x-4">
          ...
          <div className="flex flex-col">
            <span className="text-2xl font-medium text-primary dark:text-neutral-100">
              {data?.name}
            </span>
