...
const Item = ({ data, type }: ItemProps) => (
  <div className="flex flex-col items-center gap-y-1">
    ...
    <span className="text-3xl text-primary dark:text-neutral-100">
...
const ItemHovered = ({ data, type }: ItemHoveredProps) => (
  <div className="flex flex-col items-center gap-y-1 text-xs">
    ...
    <span className="text-primary dark:text-neutral-100">
      {`${Math.round(data?.maxWpm.wpm)} wpm`}
    </span>
    <span className="text-primary dark:text-neutral-100">{`${Math.floor(data?.maxWpm.raw)} raw`}</span>
    <span className="text-primary dark:text-neutral-100">{`${Math.floor(data?.maxWpm.acc)}% acc`}</span>
    <span className="text-primary dark:text-neutral-100">{`${Math.floor(data?.maxWpm.consistency)}% con`}</span>
